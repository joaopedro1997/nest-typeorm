...
let UsersService = class UsersService {
    ...
    async update(id, updateUsersDto) {
        ...
        if (!updateResult.affected) {
            throw new typeorm_2.EntityNotFoundError(users_entity_1.User, id);
        }
    }
    async remove(id) {
        const deleteResult = await this.UserRepo.delete({ id });
        if (!deleteResult.affected) {
            throw new typeorm_2.EntityNotFoundError(product_entity_1.Product, id);
        }
    }
};
